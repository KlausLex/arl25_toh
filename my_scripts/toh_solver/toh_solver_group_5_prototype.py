...
def execute_motion(dmp_gen, dmp_save_path, cube_name, position_offset, publisher, 
    ...
    trajectory, IK_joint_trajectory, gripper_traj, T = dmp_gen.compute_IK_trajectory(
        trajectory, T, subsample_factor=10) # subsample_factor originally = 3
...
def get_cubes_peg_position(peg_positions):
    ...
    cube_positions = {
        "green_cube": get_cube_position("green_cube", timeout=3.0),
        "red_cube": get_cube_position("red_cube", timeout=3.0),
        "blue_cube": get_cube_position("blue_cube", timeout=3.0),
    }
...
if __name__ == "__main__":
    ...
    # Get the MoveSet of the solving algorithm of the Tower of Hanoi
    moveset = extract_all_move_tags(answer)
    ...
    #PEG_POSITIONS = {
    #    "A": [0.2, -0.1, 0.0],  # x, y, z of stacked cube position
    #    "B": [0.23,  0.0, 0.0],  # middle position
    #    "C": [0.2,  0.1, 0.0]   # rightmost position
    #}
    ...
    # Disk numbers mapped to cube names
    DISK_TO_CUBE = {
        "1": "blue_cube",
        "2": "red_cube",
        "3": "green_cube"
    ...
    # Dictionary for the peg states
    PEG_STATES = {
        "A": ["green_cube", "red_cube", "blue_cube"],
        "B": [],
        "C": []
    }

    for current_move in moveset:
        pick_offset = [0.0, 0.0, 0.1]  # Slight offset above cube for picking
        place_offset = [0.0, 0.0, 0.01]  # Slight offset above peg for placing
        current_state = get_cubes_peg_position(peg_positions)

        # 1. Parse the current move code
        disk = current_move[2]
        from_peg = current_move[3]
        to_peg = current_move[4]

        # map the left, front, and right motions according to the current necesssary trajectory
        if from_peg == 'A':
        ...
        pick_cube_name = DISK_TO_CUBE[disk]
